...
// Data is an opaque representation of data backed by memory or a ReaderAt.
type Data struct {
	mem []byte
	r   io.ReaderAt
	sz  int
...
func NewDataFile(f *os.File) (*Data, error) {
	...
	return &Data{r: f, sz: int(fInfo.Size())}, nil
}
...
func (d *Data) Slice(start, end int) *Data {
	if d.mem != nil {
		return &Data{mem: d.mem[start:end]}
	}
	return &Data{
